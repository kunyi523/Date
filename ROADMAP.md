...
- [x] **3. 卡池双语 + 按收件人语言显示** ✓ 2026-09-04
  48 张手写卡加 `te/de`，16 种真实地点描述加 `de`，`ENDINGS_EN`（按下标对应）、`SWEETS_EN`，全部手写不是翻译。
  排程把 `te/de` 带进站点、进分享链接；渲染统一走 `cT()/cD()/endingText()` **按看的人的 `LANG` 取**——
  他中文排、她英文看，是同一份计划。老链接没有 `te/de` 就回落中文。`ta()` 英文下没名字时是 them。
  事实那一行（步行 x 分钟 / 营业到 / 菜系）也按 `LANG` 出英文，但它是排程时按发件人语言算好存进 `x` 的，
