...
这份文件同时是**给自动运行的代理的操作手册**。人（坤怿）2026-09-04 起手术休养，几周不在。
...
2. 读这份文件。看第四节，找**第一条**未勾选、且"依赖"已满足的任务
3. 开分支 `cursor/<任务名>-cb25`，只做这一条，做小做完
4. 跑 `test/`（第 1 条任务做完之后才有）。跑不过就修，修不好就写进第五节日志、不合并
5. 提交、推送、开 PR。PR 正文写：做了什么、怎么验的、没做什么
6. **如果第二节的授权为"是"且 CI 绿**：合并到 main（GitHub Pages 自动上线）。否则只留 PR
7. 在第四节把这条勾掉，在第五节写一行日志（日期、做了什么、下次注意什么）
...
- 动 `kunyi523/xindong` 仓库（Worker 的部署副本），除非第二节写了可以
- 让旧链接失效：`?s=` `?c=` `?shop=` 必须一直能打开
- 不确定就开 PR 不合并，把问题写进 PR 正文和第五节
...
1. **一对一链接**（最强）：她点完"愿意"，下一步就是"下次换你排"，他的名字已填好。
   每一次收到都是一次产品体验，发生在情绪最高点。→ 已做（第四节第 0 条）
...
| 项 | 状态 | 说明 |
|---|---|---|
| 代理可以把 CI 绿的 PR 合并到 main | **否**（等人在聊天里说"可以合并"后改成"是"） | 合并 = 上线 |
...
- [ ] **4. 短链 + 链接预览（og:image）**
  ...
  依赖：1，**且第二节 Worker 部署授权为"是"或构建源已改**
